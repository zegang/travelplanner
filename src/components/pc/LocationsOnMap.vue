<template>
  ...
  <v-row no-gutters class="h-100">
    <!-- Collapsible Location List (Left) -->
    ...
    >
      <v-card class="pa-4 rounded-xl elevation-4 h-100">
        <v-card-title class="d-flex align-center justify-space-between">
          <span class="text-h5 font-weight-bold">旅行目的地🗺️</span>
          <v-btn
            icon
            size="small"
            variant="text"
          ...
          >
            <v-icon>{{ mdiArrowCollapseVertical }}</v-icon>
          </v-btn>
        </v-card-title>
    ...
    <!-- Baidu Map View (Right) -->
    <v-col
      :cols="isNavCollapsed ? 12 : 12"
      :sm="isNavCollapsed ? 12 : 7"
      :md="isNavCollapsed ? 12 : 8"
      :lg="isNavCollapsed ? 12 : 9"
      class="pa-4 h-100"
      order-sm="2"
    >
...
import {
  mdiArrowCollapseVertical, mdiCar, mdiChevronLeft,
  mdiChevronRight, mdiMapMarkerPath
} from '@mdi/js';
