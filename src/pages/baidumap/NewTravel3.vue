      ...
      <v-row no-gutters class="h-100">
        ...
        <v-col :cols="isNavCollapsed ? 12 : 12" :sm="isNavCollapsed ? 12 : 7" :md="isNavCollapsed ? 12 : 8" :lg="isNavCollapsed ? 12 : 9" class="pa-4 h-100">
          <v-card class="pa-2 rounded-xl h-100 elevation-4 d-flex flex-column">
            ...
            >
              <v-icon>{{ isNavCollapsed ? 'mdi-chevron-right' : 'mdi-chevron-left' }}</v-icon>
            </v-btn>
...
baiduScript.src = 'https://api.map.baidu.com/api?v=3.0&ak=' + YOUR_BAIDU_MAPS_API_KEY +'&callback=initBaiduMap';
...
const initBaiduMap = () => {
  ...
  const toBaiduPoint = (coords: { lat: number, lng: number }) => {
  ...
  const baiduPoints = travelLocations.value.map(loc => toBaiduPoint(loc.coordinates));
  
  // Set the map viewport to fit all points
  if (baiduPoints.length > 0) {
    mapInstance.value.setViewport(baiduPoints);
  }

  // Create markers and add click listeners
  markers.value = travelLocations.value.map((location, index) => {
    const point = toBaiduPoint(location.coordinates);
  ...
  if (baiduPoints.length > 1) {
    ...
    walkingRoute.search(baiduPoints[0], baiduPoints[baiduPoints.length - 1]);
  }
};
